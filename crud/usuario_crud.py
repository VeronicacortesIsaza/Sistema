...
class UsuarioCRUD:
    ...
    def crear_usuario(db: Session, nuevo_usuario: Usuario):
        ...
        if nuevo_usuario.tipo_usuario not in tipos_usuarios:
            raise ValueError(f"Tipo de usuario inválido. Debe ser uno de: {tipos_usuarios}")
        ...
        if existente:
            raise ValueError(f"El nombre de usuario '{nuevo_usuario.nombre_usuario}' ya está en uso.")
    ...
    def obtener_usuarios_admin(db: Session):
        ...
        try:
            admis = db.query(Usuario).filter(Usuario.tipo_usuario.ilike("Administrador")).all()
            if not admis:
    ...
    def obtener_usuarios_cliente(db: Session):
        ...
        try:
            clientes = db.query(Usuario).filter(Usuario.tipo_usuario.ilike("Cliente")).all()
            if not clientes:
