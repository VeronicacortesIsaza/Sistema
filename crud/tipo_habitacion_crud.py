...
class TipoHabitacionCRUD:
    ...
    def crear_tipo_habitacion(db: Session, tipo: Tipo_Habitacion):
        ...
        existente = db.query(Tipo_Habitacion).filter(Tipo_Habitacion.nombre_tipo == tipo.nombre_tipo).first()
        if existente:
        ...
        if not tipo.descripcion.strip():
            raise ValueError("La descripción del tipo de habitación no puede estar vacía")
        if tipo.id_usuario_crea is None:
            raise ValueError("El ID del usuario que crea el tipo de habitación es obligatorio")
        usuario_existe = db.query(Usuario).filter(Usuario.id_usuario == tipo.id_usuario_crea).first()
    ...
    def actualizar_tipo_habitacion(db, id_tipo, **campos_actualizacion):
        tipo = db.query(Tipo_Habitacion).filter(Tipo_Habitacion.id_tipo == id_tipo).first()
        if not tipo:
            raise ValueError("Tipo de habitación no encontrado")
        ...
        existente = (
            db.query(Tipo_Habitacion).filter(Tipo_Habitacion.nombre_tipo == nuevo_nombre, Tipo_Habitacion.id_tipo != id_tipo).first()
        )
        if existente:
